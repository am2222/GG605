    ...
    for feat in lyr:
        ...
        if len(cities) != 1:
            ...
            if feat.get('Is_Main_Hu') == "T":
                main_hu = True
            Hub(name=str(feat.get('Name')), city=cities[0], is_main=main_hu, geom=feat.geom.wkb,
                transit_mode=int(feat.get('Transit_Mo'))).save()
...
def import_cost(request):
    with open('home/m/605/cost.csv', 'rt') as csvfile:
        ...
        for row in reader:
            ...
            persona = (row['persona'])
            cost = ((row['Total_Cost']))
            # c_t=City.objects.get(name__contains=c_to)
            # c_f=City.objects.get(name__contains=c_from)

            try:
                r = Route.objects.get(city_to=c_to, city_from=c_from, persona_id=persona, transit_mode=1)
                r.total_cost = cost
                r.save()
                print(r.id, "is updated to", r.total_cost)
            except:
                print("no route for", c_from.name, c_to.name)
...
def importprivate(request):
    ...
    with open('home/m/605/private4.csv', 'rt') as csvfile:
        ...
        for row in reader:
            ...
            if row['stop 4 lat'] != "" and row['stop 4 long'] != "" and row['stop 4 lat'] != '"' and row[
                'stop 4 long'] != '"':
...
def app_view(request):
    ...
    return render(request, "map.html",
                  {'title': 'title', 'cities': cities, 'personas': personas})


def generatematrixes(request):


    routs=Route.objects.filter(transit_mode=2)
    for r in routs:
        r.total_cost=(r.total_distance/1000)*0.58
        r.save()

    routs = Route.objects.filter(transit_mode=3)
    for r in routs:
        r.total_time = (r.total_time*60)
        r.save()
            ...
            for c in citys:
                ...
                for r in routes:
                    res = {
                        ...
                        "c2_lat": c.geom.centroid[0],
                        "c2_long": c.geom.centroid[1],
                ...
                for mode in persona.mode.split(','):
                    ...
                    if len(h1) == 1 and len(h2) == 1:
                        ...
                        start_time, end_time, main_time, end_distance, start_distance, main_distance, coords, line = \
                            route((h1[0].geom.centroid[1], h1[0].geom.centroid[0]),
                                  (h2[0].geom.centroid[1], h2[0].geom.centroid[0]), m, leave_at, time, day)
